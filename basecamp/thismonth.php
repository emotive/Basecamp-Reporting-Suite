...
<h1>Hours This Month</h1>
<?php

$startdate = date('Y-m-d', mktime(0, 0, 0, date('m'), 1, date('Y')));
$enddate = date('Y-m-d', mktime(0, 0, 0, date('m'), 31, date('Y')));
    ...
    foreach($response['body']->person as $person)
    	...
    	if ($person->{'company-id'} == $company_id) {    
			...
			echo "<div class='person'>";
      		echo $person->{'first-name'}." ".$person->{'last-name'}."<br><span class='number'>".round($totalhours,2)."</span><br><span class='summary'>".round(($totalhours/($business_hours*20.91)*100),1)."% complete</span>";
      		echo "</div>";
...
echo "company total<br><span class='number'>".round($companyhours,2)."</span><br><span class='summary'>".round(($companyhours/($business_hours*20.91*$totalemployees)*100),1)."% complete</span>";
echo "</div>";
